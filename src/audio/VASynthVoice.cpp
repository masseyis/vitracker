...
bool VASynthVoice::isActive() const {
...
void VASynthVoice::updateOscFrequencies(float baseFreq, const model::VAParams& params) {
    // OSC1
...
float VASynthVoice::processSample(const model::VAParams& params, float pitchMod, float cutoffMod) {
    // Apply glide
    ...
    float lfoPitchMod = 1.0f + lfoValue * params.lfo.toPitch * 0.5f;  // +/- 50% = 1 octave
