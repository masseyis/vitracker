...
const char *App::grooveNames_[5] = {"Straight", "Swing 50%", "Swing 66%",
                                    "MPC 60%", "Humanize"};
...
App::App() {
  ...
  keyHandler_->onPlayStop = [this]() {
    if (audioEngine_.isPlaying()) {
      ...
      if (auto *patternScreen =
              dynamic_cast<ui::PatternScreen *>(screens_[2].get())) {
  ...
  // Create screens (1=Song, 2=Chain, 3=Pattern, 4=Instrument, 5=Channel,
  // 6=Mixer)
  ...
  audioSettingsPopup_ =
      std::make_unique<ui::AudioSettingsPopup>(deviceManager_);
  addChildComponent(audioSettingsPopup_.get());

  // Initialize Tip Me button (mouse-only, no keyboard focus)
  tipMeButton_.setColour(juce::TextButton::buttonColourId,
                         juce::Colour(0xffff5e5b));
  tipMeButton_.setColour(juce::TextButton::textColourOffId,
                         juce::Colours::white);
  ...
  if (auto *patternScreen =
    ...
    patternScreen->onChordPreview = [this](const std::vector<int> &notes,
                                           int instrument) {
      if (audioEngine_.isPlaying())
        return;
  ...
  if (auto *instrumentScreen =
          dynamic_cast<ui::InstrumentScreen *>(screens_[3].get())) {
  ...
  if (auto *songScreen = dynamic_cast<ui::SongScreen *>(screens_[0].get())) {
    songScreen->onJumpToChain = [this](int chainIndex) {
      if (auto *chainScreen =
              dynamic_cast<ui::ChainScreen *>(screens_[1].get())) {
    ...
    songScreen->onNewProject = [this]() { confirmNewProject(); };
  ...
  if (auto *patternScreen =
    ...
    patternScreen->onJumpToInstrument = [this](int instrumentIndex) {
      ...
      if (auto *channelScreen =
              dynamic_cast<ui::ChannelScreen *>(screens_[4].get())) {
  ...
  // Enter key confirms/activates (e.g., jump to chain from song screen, pattern
  // from chain screen)
  keyHandler_->onConfirm = [this]() {
    // Song screen -> Chain screen
    if (auto *songScreen =
            dynamic_cast<ui::SongScreen *>(screens_[currentScreen_].get())) {
    ...
    else if (auto *chainScreen = dynamic_cast<ui::ChainScreen *>(
                 screens_[currentScreen_].get())) {
    ...
    else if (auto *patternScreen = dynamic_cast<ui::PatternScreen *>(
                 screens_[currentScreen_].get())) {
  ...
  modeManager_.onModeChanged = [this](input::Mode newMode) {
    if (newMode != input::Mode::Visual) {
      if (auto *ps =
              dynamic_cast<ui::PatternScreen *>(screens_[currentScreen_].get()))
        ps->clearSelection();
    }
    repaint();
  };
  ...
  // File operations
  keyHandler_->onSave = [this](const std::string &filename) {
    saveProject(filename);
  };
  keyHandler_->onLoad = [this](const std::string &filename) {
  ...
  keyHandler_->onSavePreset = [this](const std::string &name) {
    if (auto *instrumentScreen =
      ...
      if (instrument) {
        ...
        if (presetManager_.saveUserPreset(name, engine,
  ...
  keyHandler_->onDeletePreset = [this](const std::string &name) {
    if (auto *instrumentScreen =
      ...
      if (instrument) {
        ...
        if (presetIdx >= 0 &&
            presetManager_.isFactoryPreset(engine, presetIdx)) {
  ...
  keyHandler_->onCreateSampler = [this]() {
    ...
    if (instrument) {
      ...
      if (name.find("Instrument") == 0 || name.find("Slicer") == 0 ||
          name.find("Plaits") == 0) {
  ...
  keyHandler_->onCreateSlicer = [this]() {
    ...
    if (instrument) {
      ...
      // Only rename if it has a default name
      std::string name = instrument->getName();
      if (name.find("Instrument") == 0 || name.find("Sampler") == 0 ||
          name.find("Plaits") == 0) {
    ...
    if (auto *instScreen =
      ...
      if (inst && inst->getType() == model::InstrumentType::Slicer) {
        if (auto *slicer = audioEngine_.getSlicerProcessor(currentInst)) {
          ...
          instScreen
              ->updateSlicerDisplay(); // Sync waveform display with new slices
        }
      }
    }
  };
...
void App::timerCallback() {
  ...
  // Release preview notes after timeout (release all tracks used for chord
  // preview)
...
void App::paint(juce::Graphics &g) {
  ...
  if (!screens_[currentScreen_]) {
    ...
    static const char *screenNames[] = {"SONG",       "CHAIN",   "PATTERN",
                                        "INSTRUMENT", "CHANNEL", "MIXER"};
...
bool App::keyPressed(const juce::KeyPress &key,
  ...
  bool screenInTextEdit =
      screens_[currentScreen_] && screens_[currentScreen_]->getInputContext() ==
                                      input::InputContext::TextEdit;
  if (key.getTextCharacter() == 't' &&
      modeManager_.getMode() == input::Mode::Normal && !screenInTextEdit) {
...
void App::switchScreen(int screenIndex) {
  if (screenIndex < 0 || screenIndex >= 6)
    return; // Now 6 screens
  if (screenIndex == currentScreen_)
    return;
...
void App::drawStatusBar(juce::Graphics &g, juce::Rectangle<int> area) {
  ...
  g.setColour(juce::Colours::white);
  juce::String modeStr =
      tempoAdjustMode_ ? "TEMPO" : modeManager_.getModeString();
  g.drawText("-- " + modeStr + " --", area.removeFromLeft(120),
             juce::Justification::centredLeft, true);

  // Screen indicator (after mode)
  static const char *screenKeys[] = {"1:SNG", "2:CHN", "3:PAT",
                                     "4:INS", "5:CHS", "6:MIX"};
  for (int i = 0; i < 6; ++i) {
    g.setColour(i == currentScreen_ ? juce::Colours::yellow
                                    : juce::Colours::grey);
    g.drawText(screenKeys[i], area.removeFromLeft(50),
               juce::Justification::centred, true);
  }
  ...
  juce::String transportText = isPlaying ? "PLAYING" : "STOPPED";
  g.drawText(transportText, area.removeFromLeft(130),
             juce::Justification::centred, true);
  ...
  // Groove
  g.setColour(juce::Colours::white);
  g.drawText(project_.getGrooveTemplate(), area.removeFromRight(90),
...
void App::saveProject(const std::string &filename) {
  if (filename.empty()) {
    auto chooser = std::make_shared<juce::FileChooser>(
        "Save Project",
        juce::File::getSpecialLocation(juce::File::userHomeDirectory), "*.vit");
...
void App::hideHelp() { helpPopup_.hide(); }
...
juce::File App::getProjectFilePath() const {
  ...
  auto dir =
      currentProjectFile_.exists()
          ? currentProjectFile_.getParentDirectory()
          : juce::File::getSpecialLocation(juce::File::userDocumentsDirectory)
                .getChildFile("Vitracker Projects");
...
void App::confirmNewProject() {
  if (projectDirty_) {
    ...
    auto options = juce::MessageBoxOptions()
                       ...
                       .withMessage("Current project has unsaved changes. "
