...
// Debug logging for dx7note
#define DX7NOTE_DEBUG 1
#if DX7NOTE_DEBUG
static int dx7note_log_counter = 0;
#define DX7NOTE_LOG(msg) do { \
    if (dx7note_log_counter++ < 50) { \
        std::cerr << "[DX7Note] " << msg << std::endl; \
    } \
} while(0)
#else
#define DX7NOTE_LOG(msg)
#endif
...
void Dx7Note::init(const uint8_t patch[156], int midinote, int velocity, int channel, const Controllers *ctrls) {
    ...
    DX7NOTE_LOG("init() called: midinote=" << midinote << " velocity=" << velocity);
    DX7NOTE_LOG("  algorithm=" << (int)patch[134] << " feedback=" << (int)patch[135]);
    ...
    for (int op = 0; op < 6; op++) {
        ...
        outlevel = outlevel << 5;
        int kvs = patch[off + 15];
        int velScale = ScaleVelocity(velocity, kvs);
        outlevel += velScale;
        outlevel = max(0, outlevel);
        int rate_scaling = ScaleRate(midinote, patch[off + 13]);
        DX7NOTE_LOG("  Op" << (6-op) << ": rawOL=" << rawOL << " scaledOL=" << outlevel
                    << " KVS=" << kvs << " velScale=" << velScale
                    << " R1-4=" << rates[0] << "," << rates[1] << "," << rates[2] << "," << rates[3]);
...
void Dx7Note::compute(int32_t *buf, int32_t lfo_val, int32_t lfo_delay, const Controllers *ctrls) {
    ...
    // ==== OP RENDER ====
    static int compute_log_counter = 0;
    bool should_log = (compute_log_counter++ < 10);
    ...
    for (int op = 0; op < 6; op++) {
        if ( ctrls->opSwitch[op] == '0' )  {
            ...
            if (should_log) {
                DX7NOTE_LOG("compute() Op" << (6-op) << " DISABLED by opSwitch");
            }
        } else {
            ...
            if (should_log) {
                DX7NOTE_LOG("compute() Op" << (6-op) << " level_in=" << level
                            << " freq=" << params_[op].freq << " basepitch=" << basepitch);
            }
        }
    }

    if (should_log) {
        DX7NOTE_LOG("compute() calling render: algo=" << algorithm_ << " fb_shift=" << fb_shift_);
    }
