...
// Debug logging for FmCore
#define FMCORE_DEBUG 1
#if FMCORE_DEBUG
static int fmcore_log_counter = 0;
#define FMCORE_LOG(msg) do { \
    if (fmcore_log_counter++ < 200) { \
        std::cerr << "[FmCore] " << msg << std::endl; \
    } \
} while(0)
#else
#define FMCORE_LOG(msg)
#endif
...
void FmCore::render(int32_t *output, FmOpParams *params, int algorithm, int32_t *fb_buf, int feedback_shift) {
    ...
    FMCORE_LOG("render() algo=" << algorithm << " fb_shift=" << feedback_shift);
    ...
    for (int op = 0; op < 6; op++) {
        ...
        FMCORE_LOG("  Op" << op << ": level_in=" << param.level_in
                   << " gain1=" << gain1 << " gain2=" << gain2
                   << " thresh=" << kLevelThresh << " pass=" << (gain1 >= kLevelThresh || gain2 >= kLevelThresh));
